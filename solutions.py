...
def is_sweltering():
    ...
    """
    temperature_hot = input('What is the current temperature in Farenheit? ')
    if temperature_hot.isnumeric():
        if 90 < int(temperature_hot):
            return True
        else:
            return False
    else:
...
def is_warm():
    ...
    """
    temperature_warm= input('What is the current temperature in Farenheit? ')
    if temperature_warm.isnumeric():
        temperature_warm = int(temperature_warm)
        if 75<= temperature_warm and temperature_warm <= 87:
            return True
        else:
            return False
...
def is_humid():
    ...
    """
    weather_humid = input('Is it humid today? ')
    weather_humid = weather_humid.lower()
    response = ['yes']
    if weather_humid in response:
        return True
    else:
        return False
...
def is_inclement():
    ...
    weather_forecast = weather_forecast.lower()
    conditions = ['rain', 'snow', 'sleet']
    if weather_forecast in conditions:
        return True
    else:
        return False
